...
interface TournamentsPanelProps {
  ...
  renderSection?: (
    section: Extract<CommandCenterSection, "groups" | "leagues">,
  ) => React.ReactNode;
  ...
  createGameType?: "tournament" | "cash";
...
export default function TournamentsPanel({
  ...
  const [initialGameType, setInitialGameType] = useState<"tournament" | "cash">(
    "tournament",
  );
  ...
  const [showFullUpcomingSchedule, setShowFullUpcomingSchedule] = useState(
    () => new URLSearchParams(location.search).get("schedule") === "games",
  );
  const [historyCommunityFilter, setHistoryCommunityFilter] = useState("all");
  const [localSection, setLocalSection] =
    useState<CommandCenterSection>("upcoming");
  const lastCreateGameRequestRef = useRef(0);
  const lastHomeRequestRef = useRef(homeRequestId);
  const lastGamesRequestRef = useRef(gamesRequestId);
  const creationRouteRef = useRef<string | null>(null);
  ...
  const {
    data: leagueEvents = [],
    isLoading: loadingLeagueEvents,
    error: leagueScheduleError,
    refetch: refetchLeagueEvents,
  } = useQuery({
  ...
  const registerMutation = useMutation({
    mutationFn: (tournament: Tournament) =>
      tournament.groupid
        ? api.groupRegister(tournament.tournamentid)
        : api.selfRegister(tournament.tournamentid),
          ...
          item.tournamentid === tournament.tournamentid
            ? {
                ...
                playercount: Math.max(
                  0,
                  Number(item.playercount ?? 0) + (item.isregistered ? 0 : 1),
                ),
              }
            : item,
        ),
            ...
            ? {
                ...
                playercount: Math.max(
                  ...
                  Number(game.playercount ?? 0) +
                    (variables.status === "going" && !game.isregistered
                      ? 1
                      : variables.status === "not_going" && game.isregistered
                        ? -1
                        : 0),
                ),
              }
  ...
  const rsvpError =
    ...
    cashRsvpMutation.error?.message;

  const leagueScheduleItems = useMemo(
    () => buildLeagueScheduleItems(leagueEvents),
    [leagueEvents],
  );
  const gameScheduleItems = useMemo(
    () => games.map(gameToScheduleItem),
    [games],
  );
  const tournamentScheduleItems = useMemo(
    () => mine.map(tournamentToScheduleItem),
    [mine],
  );
  const allScheduleItems = useMemo(() => {
    const tournamentIds = new Set(
      tournamentScheduleItems.flatMap((item) =>
        item.kind === "tournament" ? [item.tournament.tournamentid] : [],
      ),
    );
    const linkedIds = new Set(
      leagueScheduleItems.flatMap((item) =>
        item.kind === "league" && item.tournamentId ? [item.tournamentId] : [],
      ),
    );
    return [
      ...tournamentScheduleItems.filter(
        (item) =>
          item.kind !== "tournament" ||
          !linkedIds.has(item.tournament.tournamentid),
      ),
      ...leagueScheduleItems,
      ...gameScheduleItems.filter(
        (item) =>
          item.kind !== "cash" ||
          !item.game.tournamentid ||
          (!linkedIds.has(item.game.tournamentid) &&
            !tournamentIds.has(item.game.tournamentid)),
      ),
    ];
  ...
  const homeSchedule = useMemo(
    () => buildHomeSchedule({ tournaments: mine, games, leagueEvents }),
    [mine, games, leagueEvents],
  );
  const historyScheduleItems = useMemo(
    () =>
      allScheduleItems
        .filter((item) => !isUpcomingScheduleItem(item))
        .sort(compareScheduleItems)
        .reverse(),
    [allScheduleItems],
  );
  const historyCommunityOptions = useMemo(() => {
    const options = new Map<
      string,
      { value: string; label: string; type: "Group" | "League" }
    >();
  ...
  const filteredHistoryScheduleItems = useMemo(
    () =>
      historyCommunityFilter === "all"
        ? historyScheduleItems
        : historyScheduleItems.filter(
            (item) => getScheduleCommunityKey(item) === historyCommunityFilter,
          ),
    [historyCommunityFilter, historyScheduleItems],
  );
  const scheduleList =
    scheduleView === "history"
      ? filteredHistoryScheduleItems
      : upcomingScheduleItems;
  const firstHostedTournament =
    mine.find(
      (tournament) =>
        isUpcomingScheduleItem(tournamentToScheduleItem(tournament)) &&
        tournament.ownerid === me?.guid,
    ) ??
    mine.find((tournament) =>
      isUpcomingScheduleItem(tournamentToScheduleItem(tournament)),
    ) ??
    null;
  const hostableGroups = useMemo(
    () => groups.filter((group) => group.isadmin && group.approved),
    [groups],
  );
  const loadingSchedule =
    loadingMine ||
    loadingGames ||
    (loadingLeagueEvents && scheduleList.length === 0);
  const externalSection =
    activeSection === "groups" || activeSection === "leagues"
  ...
  useEffect(() => {
    if (
      !createGameRequestId ||
      createGameRequestId === lastCreateGameRequestRef.current
    )
      return;
    lastCreateGameRequestRef.current = createGameRequestId;
    creationRouteRef.current = `${location.pathname}${location.search}`;
    setInitialGameType(createGameType);
    setShowCreate(true);
  }, [createGameRequestId, createGameType, location.pathname, location.search]);

  useEffect(() => {
    if (
      showCreate &&
      creationRouteRef.current &&
      creationRouteRef.current !== `${location.pathname}${location.search}`
    ) {
      setShowCreate(false);
    }
  }, [location.pathname, location.search, showCreate]);

  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const routeSection = params.get("section");
    if (routeSection && routeSection !== "upcoming") return;
    const mode = params.get("schedule") === "games" ? "games" : "home";
    setShowFullUpcomingSchedule(mode === "games");
    onScheduleModeChange?.(mode);
  }, [location.search, onScheduleModeChange]);
  ...
  useEffect(() => {
    if (!gamesRequestId || gamesRequestId === lastGamesRequestRef.current)
      return;
  ...
  useEffect(() => {
    ...
    if (
      !historyCommunityOptions.some(
        (option) => option.value === historyCommunityFilter,
      )
    ) {
  ...
  function openHostGame(type: "tournament" | "cash" = "tournament") {
    if (hostableGroups.length > 0) {
      creationRouteRef.current = `${location.pathname}${location.search}`;
      setInitialGameType(type);
      setShowCreate(true);
      return;
    }
  ...
  function openScheduleItem(item: ScheduleItem) {
    if (item.kind === "tournament") {
      navigate(
        item.canManage
          ? `/tournament/${item.tournament.tournamentid}`
          : `/lobby/${item.tournament.tournamentid}`,
        item.canManage
          ? {
              state: {
                tab: "run",
                ...(demoMode ? { demoCoach: "start" } : {}),
              },
            }
          : undefined,
      );
  ...
  if (showCreate) {
    return (
      <CreateTournamentComposer
        ...
        error={
          createMutation.error?.message || createGameMutation.error?.message
        }
      />
    );
  }
  ...
  return (
    <>
      ...
      {externalSection ? (
        ...
        />
      ) : activeSection === "upcoming" && !showFullUpcomingSchedule ? (
        <HomeDashboard
          me={me}
          demoMode={demoMode}
          next={homeSchedule.next}
          upcoming={homeSchedule.upcoming}
          groups={groups}
          leagues={leagues}
          scheduleLoading={
            loadingMine ||
            loadingGames ||
            loadingLeagues ||
            (leagues.length > 0 && loadingLeagueEvents)
          }
          scheduleErrors={[
            ...(mineError
              ? [
          ...
          onHost={openHostGame}
          onCreateGroup={onStartGroupCreate ?? (() => changeSection("groups"))}
          onCreateLeague={
        ...
        (loadingSchedule ||
          ...
          scheduleView === "history") && (
          <>
            {(scheduleView === "history" || showFullUpcomingSchedule) && (
              <nav
                aria-label="Games views"
                className="mb-5 flex items-center gap-2 border-b border-pit-border pb-3"
              >
                <Link
                  to="/?section=upcoming&schedule=games"
                  aria-current={
                    scheduleView === "upcoming" ? "page" : undefined
                  }
                  className={`rounded-lg border px-4 py-3 text-sm font-semibold ${scheduleView === "upcoming" ? "border-pit-teal/50 bg-pit-teal/10 text-pit-teal" : "border-pit-border text-pit-text"}`}
                >
                  Upcoming
                </Link>
                <Link
                  to="/?section=history"
                  aria-current={scheduleView === "history" ? "page" : undefined}
                  className={`rounded-lg border px-4 py-3 text-sm font-semibold ${scheduleView === "history" ? "border-pit-teal/50 bg-pit-teal/10 text-pit-teal" : "border-pit-border text-pit-text"}`}
                >
                  Past
                </Link>
              </nav>
            ...
            {leagueScheduleError && (
              <p className="mb-3 rounded-lg border border-yellow-300/20 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-100">
                League games could not load yet. Tournament rows are still
                shown.
              </p>
            )}
            {loadingSchedule ? (
              ...
              <section>
                <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                  <div className="flex items-center justify-between gap-3 sm:block">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-pit-teal">
                      Past games
                    </p>
                    <span className="rounded-full border border-pit-border bg-pit-surface px-2.5 py-1 text-xs font-semibold text-[#d0d0da] sm:hidden">
                      {historyCommunityFilter === "all"
                        ? historyScheduleItems.length
                        : `${filteredHistoryScheduleItems.length} of ${historyScheduleItems.length}`}
                    </span>
                  </div>
                  <div className="flex items-end gap-2">
                    <label className="min-w-0 flex-1 sm:w-72 sm:flex-none">
                      <span className="mb-1 block text-[10px] font-semibold uppercase tracking-[0.16em] text-[#b8b8c7]">
                        Group or league
                      </span>
                      <select
                        ...
                        onChange={(event) =>
                          setHistoryCommunityFilter(event.target.value)
                        }
                ...
                <ScheduleList
                  ...
                  onRegister={(tournament) =>
                    registerMutation.mutate(tournament)
                  }
                  onDecline={(tournament) =>
                    declineMutation.mutate(tournament.tournamentid)
                  }
                  onLeagueEvent={(item) =>
                    navigate(
                      `/league/${encodeURIComponent(item.leagueId)}/event/${encodeURIComponent(item.eventId)}`,
                  ...
                  onCashRsvp={(item, status) =>
                    cashRsvpMutation.mutate({ gameId: item.game.id, status })
                  }
                />
              </section>
...
function DashboardOverview({
  ...
  const hasInvitedPlayer = adminGroups.some(
    (group) => Number(group.membercount ?? 0) > 1,
  );
  ...
  return (
    <section className="mb-5 max-w-[22rem]">
      <FirstRunSetupCard
        ...
        onInvitePlayer={() =>
          primaryAdminGroup && onStartGroupInvite(primaryAdminGroup.groupid)
        }
        onSkipInvite={onSkipInvite}
        onCreateGame={onStartFirstGame}
        onOpenFirstGame={() =>
          firstHostedTournament &&
          onOpenFirstGame(firstHostedTournament.tournamentid)
        }
  ...
  return (
    <aside className="rounded-xl border border-pit-teal/25 bg-[radial-gradient(circle_at_top_left,rgba(20,184,181,0.18),transparent_38%),linear-gradient(150deg,rgba(18,46,48,0.92),rgba(24,24,30,0.96))] p-3 shadow-[0_16px_42px_rgba(0,0,0,0.2)] sm:p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-pit-teal">
            First night setup
          </p>
      ...
      <p className="mt-2 text-xs leading-5 text-pit-text">
        Start with a group, invite someone if you want, then build the first
        tournament. This coach disappears once you finish it.
      </p>
        ...
        {steps.map((step, index) => (
          <div
            ...
            className={`rounded-lg border px-2.5 py-2 ${
              step.complete
                ? "border-pit-teal/25 bg-pit-teal/10"
                : "border-pit-border/70 bg-pit-bg/45"
            }`}
          >
            <div className="flex items-center gap-2">
              <span
                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-[10px] font-bold ${
                  step.complete
                    ? "bg-pit-teal text-pit-bg"
                    : "bg-white/8 text-pit-muted"
                }`}
              >
            ...
            <p className="mt-1 pl-7 text-[11px] leading-4 text-pit-muted">
              {step.body}
            </p>
          </div>
        ))}
      ...
      {hasFirstGame && firstGameName && (
        <p className="mt-3 rounded-lg border border-pit-teal/20 bg-pit-teal/10 px-2.5 py-2 text-[11px] leading-4 text-pit-text">
          Next stop:{" "}
          <span className="font-semibold text-white">{firstGameName}</span>.
          Open it, check players in from the Players tab, then start the clock
          from Run Tournament.
        </p>
      )}
    </aside>
...
function CommunitiesDirectory({
  ...
  const imageMutation = useMutation({
    mutationFn: async ({
      ...
      if (type === "group") {
        return api.updateGroupCommunityImage(id, {
          communityimagedata: image,
          communityimagefilename: filename,
        });
      }
      return api.updateLeagueCommunityImage(id, {
        communityimagedata: image,
        communityimagefilename: filename,
      });
    },
        ...
        queryClient.setQueryData<Group[]>(["groups"], (current = []) =>
          current.map((group) =>
            group.groupid === variables.id
              ? {
                  ...group,
                  communityimagedata: variables.image,
                  communityimagefilename: variables.filename,
                }
              : group,
          ),
        );
    ...
    onError: (error) => {
      ...
      setImageError(
        error instanceof Error
          ? error.message
          : "Community image could not be saved.",
      );
    },
  });
    ...
    () =>
      [
        ...groups.map((group) => ({
          ...
          detail: group.nexttournamentname
            ? `Next: ${group.nexttournamentname}`
            : "No game scheduled",
    ...
    <section>
      ...
      <div className="mb-4 grid grid-cols-4 divide-x divide-pit-border overflow-hidden rounded-lg border border-pit-border bg-pit-surface/75">
        {[
          ...
          <div
            key={stat.label}
            className="min-w-0 px-2 py-2.5 text-center sm:flex sm:items-center sm:justify-center sm:gap-2 sm:px-3"
          >
            <stat.icon
              size={15}
              className="mx-auto mb-1 text-pit-teal sm:m-0"
            />
            <div className="min-w-0 sm:text-left">
              <p className="text-base font-bold leading-none text-white">
                {stat.value}
              </p>
              <p className="mt-1 truncate text-[9px] font-semibold uppercase tracking-[0.08em] text-pit-text sm:text-[10px]">
                {stat.label}
              </p>
            </div>
          </div>
      ...
      {imageError && (
        <p className="mb-3 rounded-lg border border-red-400/25 bg-red-400/10 px-3 py-2 text-sm text-red-100">
          {imageError}
        </p>
      )}
      {imageSuccess && (
        <p className="mb-3 rounded-lg border border-emerald-400/25 bg-emerald-400/10 px-3 py-2 text-sm text-emerald-100">
          {imageSuccess}
        </p>
      )}
      ...
      {communities.length === 0 ? (
        <div className="rounded-xl border border-dashed border-pit-border bg-pit-surface/60 px-4 py-10 text-center">
          ...
          <p className="mt-1 text-sm text-pit-text">
            Create or join one to keep your games and seasons together.
          </p>
        ...
        <div className="grid gap-2 md:grid-cols-2 md:gap-3">
          {communities.map((community) => (
            ...
            >
              <div
                className={`relative flex h-[3.25rem] w-[3.25rem] shrink-0 items-center justify-center overflow-hidden rounded-lg border md:h-[4.25rem] md:w-[4.25rem] ${
                  community.type === "group"
                    ? "border-pit-teal/30 bg-pit-teal/10"
              ...
              >
                ...
                {community.isAdmin && (
                  ...
                  >
                    ...
                    imageMutation.variables?.id === community.id ? (
              ...
              <div className="min-w-0 self-center md:self-start">
                <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                  <h3 className="min-w-0 truncate text-sm font-bold text-white transition group-hover:text-pit-teal md:text-base">
                    {community.name}
                  </h3>
                  <span
                    className={`shrink-0 rounded-full border px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-[0.12em] ${
                      community.type === "group"
                ...
                <div className="mt-1 flex min-w-0 items-center gap-2 text-[11px] text-pit-text md:mt-2 md:text-xs">
                  <span className="inline-flex shrink-0 items-center gap-1">
                    <Users size={12} /> {community.count}
                  </span>
                  <span className="truncate">{community.detail}</span>
                </div>
                <p className="mt-2 hidden text-[10px] uppercase tracking-[0.12em] text-pit-muted md:block">
                  Join code {community.code}
                </p>
              </div>
              ...
              <button
                ...
                onClick={() =>
                  onOpen({ type: community.type, id: community.id })
                }
...
function CreateCommunityModal({
  ...
  return (
    <Modal
      title="Create Group or League"
      open={open}
      onClose={onClose}
      mobilePlacement="center"
    >
      <p className="text-sm text-pit-text">
        Choose the kind of poker community you want to set up.
      </p>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        >
          ...
          <span className="mt-1 block text-xs leading-5 text-pit-text">
            Bring players together to organize games, posts, and invites.
          </span>
        </button>
        <button
          type="button"
        ...
        >
          ...
          <span className="mt-1 block text-xs leading-5 text-pit-text">
            Set up a multi-event season with standings, points, and fees.
          </span>
        </button>
      </div>
    </Modal>
    ...
    >
      <div className="space-y-3">
        <input
          ...
          autoFocus
        />
        <p className="text-center text-xs text-pit-muted">
          Enter a group or league join code.
        </p>
      </div>
    </Modal>
  );
...
type ScheduleListProps = {
  ...
  onCashRsvp: (
    item: Extract<ScheduleItem, { kind: "cash" }>,
    status: "going" | "not_going",
  ) => void;
};
...
function ScheduleList({
  ...
  useEffect(() => {
    if (!focusItemId || !visibleItems.some((item) => item.id === focusItemId))
      return;
      ...
      <div className="divide-y divide-pit-border/60">
        {visibleItems.map((item) => (
          <ScheduleRow
            ...
            onRegister={
              item.kind === "tournament"
                ? () => onRegister(item.tournament)
                : undefined
            }
            onDecline={
              item.kind === "tournament"
                ? () => onDecline(item.tournament)
                : undefined
            }
            onLeagueEvent={
              item.kind === "league" ? () => onLeagueEvent(item) : undefined
            }
            onLeagueLobby={
              item.kind === "league" ? () => onLeagueLobby(item) : undefined
            }
            onCashRsvp={
              item.kind === "cash"
                ? (status) => onCashRsvp(item, status)
                : undefined
            }
          />
        ))}
...
function ScheduleRow({
  ...
  const showRsvp =
    view === "upcoming" &&
    isTournament &&
    Boolean(item.tournament.groupid) &&
    !item.canManage;
  const leagueEventStarted =
    isLeague && hasScheduleStarted(item.date, item.time);
  const showLeagueRsvp =
    view === "upcoming" &&
    isLeague &&
    item.isParticipant &&
    !leagueEventStarted;
  const showLeagueLobby =
    view === "upcoming" && isLeague && item.isParticipant && leagueEventStarted;
  const showLeagueAdminOnly =
    view === "upcoming" && isLeague && item.canManage && !item.isParticipant;
  const showLeagueAdminPlayerMenu =
    view === "upcoming" && isLeague && item.canManage && item.isParticipant;
  const showTournamentAdminPlayerActions =
    view === "upcoming" && isTournament && item.canManage && isRegistered;
  ...
  const showMobileStatus = Boolean(
    statusLabel && (needsRsvp || isRegistered || isDeclined),
  );
  ...
  return (
    <div
      ...
      className={`grid border-l-2 transition ${
        calendarLayout
    ...
    >
      {calendarLayout && (
        <div className="col-start-1 row-span-2 row-start-1 flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-lg border border-white/15 bg-black/30 text-center shadow-inner sm:h-16 sm:w-16">
          <span className="text-[9px] font-black uppercase tracking-[0.12em] text-[#bfc0ca]">
            {dateTile.month}
          </span>
          <span className="mt-0.5 text-xl font-black leading-none text-white sm:text-2xl">
            {dateTile.day}
          </span>
        </div>
      )}
      ...
      >
        ...
        {item.parentName && (
          <p
            className="mt-1 w-full min-w-0 truncate text-xs text-[#b3b3c2]"
        ...
        {calendarLayout && (
          <div className="mt-1.5 flex min-w-0 flex-wrap items-center gap-1.5 text-[10px] text-[#c9cad3] sm:text-[11px]">
            ...
            <span
              className={`inline-flex h-5 items-center rounded-full border px-1.5 text-[9px] font-semibold uppercase tracking-[0.08em] ${typePillClass}`}
            >
              {typeLabel}
            </span>
            {statusLabel && (
              <span
                className={`inline-flex h-5 items-center gap-1 rounded-full border px-1.5 text-[9px] font-black ${statusPillClass}`}
              >
                {statusIcon}
                {statusLabel}
              </span>
      ...
      <div
        className={
          calendarLayout ? "hidden" : "hidden lg:col-auto lg:row-auto lg:flex"
        }
      >
        <span
          className={`inline-flex h-7 items-center rounded-full border px-2.5 text-[10px] font-semibold uppercase tracking-[0.12em] ${typePillClass}`}
        >
          {typeLabel}
        </span>
      </div>
      ...
      >
        ...
        {showMobileStatus && (
          <span
            className={`inline-flex h-6 shrink-0 items-center justify-center gap-1 rounded-full border px-2 text-[9px] font-black leading-none lg:hidden ${statusPillClass}`}
          >
            {statusIcon}
            {statusLabel}
          </span>
      ...
      >
        <span
          className={`inline-flex h-6 items-center rounded-full border px-2 text-[9px] font-semibold uppercase tracking-[0.1em] ${calendarLayout ? "hidden" : "lg:hidden"} ${typePillClass}`}
        >
          {typeLabel}
        </span>
      ...
      <div
        className={
          calendarLayout
            ? "hidden"
            : "col-start-2 row-start-2 hidden items-center justify-end gap-2 lg:col-auto lg:row-auto lg:flex lg:justify-start"
        }
      >
        {fieldCount && (
        ...
        {statusLabel && (
          <span
            className={`inline-flex h-7 min-w-[6.75rem] items-center justify-center gap-1.5 whitespace-nowrap rounded-full border px-3 text-xs font-semibold ${statusPillClass}`}
          >
            {statusIcon}
            {statusLabel}
          </span>
        ...
        {showLeagueAdminPlayerMenu ? (
          <>
            <button
              ...
              onClick={onLeagueLobby}
            >
              <PlayCircle size={14} />
              <span className={calendarLayout ? "hidden sm:inline" : ""}>
                Lobby
              </span>
            </button>
            <button
          ...
          <button
            type="button"
            className="inline-flex items-center justify-center gap-2 rounded-full border border-pit-gold/55 bg-pit-gold/20 px-3 py-2 text-xs font-black text-pit-gold shadow-[0_0_18px_rgba(244,178,74,0.14)] transition hover:bg-pit-gold/30 hover:text-yellow-100"
            onClick={onLeagueEvent}
          >
          ...
          <>
            ...
            <button
              ...
              onClick={
                showCashRsvp ? () => onCashRsvp?.("not_going") : onDecline
              }
              aria-label={`Cannot attend ${item.name}`}
              title="Cannot attend"
            >
          ...
          <button
            type="button"
            className={
              item.canManage && isTournament
                ? "btn-primary gap-2 px-3 py-2 text-xs"
                : "btn-ghost px-3 py-2 text-xs !text-[#c9c9d4] hover:!text-white"
            }
            onClick={onOpen}
          >
            {item.canManage && isTournament && view === "upcoming" && (
              <PlayCircle size={14} />
            )}
            {item.canManage && isTournament
              ? view === "upcoming"
    ...
    <div className="col-span-full flex flex-col items-center justify-center gap-3 rounded-xl border border-pit-border bg-pit-surface/45 px-4 py-10">
      ...
      <div className="text-center">
        <p className="font-semibold text-white">
          {view === "history" ? "No history yet" : "No upcoming games yet"}
        </p>
        <p className="mt-1 text-sm text-pit-muted">
          {view === "history"
            ? "Completed and past-dated games will appear here."
            : "Tournaments and league events will land here once they are scheduled."}
        </p>
      </div>
    </div>
  );
...
function CreateTournamentComposer({
  ...
  groups: Group[];
  me?: Awaited<ReturnType<typeof api.me>>;
  initialGameType?: "tournament" | "cash";
  onboardingActive?: boolean;
  onBack: () => void;
  ...
  const canUseClubFeatures = Boolean(
    me?.issuperadmin ||
      me?.canuseclubfeatures ||
      me?.tierid === 2 ||
      me?.tierid === 3,
  );
  ...
  const inviteMembers = (selectedGroupDetail?.members ?? []).filter(
  ...
  useEffect(() => {
    setForm((current) => ({
      ...current,
      playerselftracking:
        canUseClubFeatures && selectedGroup?.defaulttrackingmode === "player",
    ...
    canUseClubFeatures,
    selectedGroup?.defaulttrackingmode,
  ...
  function submit(event: React.FormEvent) {
    ...
    if (form.gametype === "cash") {
      const startsat = form.tourneydate
        ? `${form.tourneydate}T${form.tourneytime || "00:00"}`
        : null;
      onSubmitCash({
        ...
        inviteUserIds:
          form.visibility === "invite_only" ? form.inviteUserIds : [],
    ...
    onSubmit({
      ...
      maxplayers:
        form.maxplayersmode === "unlimited" ? 0 : Number(form.maxplayers) || 0,
  ...
  const basicsComplete = Boolean(
    form.name.trim() &&
      form.groupid &&
      (isCashGame || (form.tourneydate && form.tourneytime)),
  );
  const rebuysActive = form.rebuysenabled;
  const addonsActive = form.addonsenabled;
  const rebuyDetailsComplete =
    !rebuysActive ||
    (Number(form.rebuyprice) >= 0 &&
      Number(form.rebuychips) > 0 &&
      Number(form.rebuylastlevel) > 0);
  const addonDetailsComplete =
    !addonsActive ||
    (Number(form.addonprice) >= 0 && Number(form.addonchips) > 0);
  const maxPlayersComplete =
    form.maxplayersmode === "unlimited" ||
    (form.maxplayersmode === "capped" && Number(form.maxplayers) > 0);
  const cashRangeComplete =
    !isCashGame ||
    !form.minbuyin ||
    !form.maxbuyin ||
    Number(form.maxbuyin) >= Number(form.minbuyin);
  const cashDetailsComplete =
    !isCashGame || (Boolean(form.stakeslabel.trim()) && cashRangeComplete);
  const inviteComplete =
    form.visibility !== "invite_only" || form.inviteUserIds.length > 0;
  const canAdvance =
    step === 0
      ? basicsComplete
      : step === 1
        ? isCashGame
          ? cashDetailsComplete
          : maxPlayersComplete && rebuyDetailsComplete && addonDetailsComplete
        : step === 2
          ? inviteComplete
          : true;
  const selectedStructure = savedStructures.find(
    (structure) => structure.id === form.savedstructureid,
  );
  const maxPlayersReview =
    form.maxplayersmode === "unlimited" ? "Unlimited" : form.maxplayers;
    ...
    <div className="fixed inset-0 z-50 mx-auto w-full overflow-y-auto bg-pit-bg px-4 pb-24 sm:static sm:z-auto sm:max-w-2xl sm:overflow-visible sm:bg-transparent sm:px-0 sm:pb-8">
      <header className="sticky top-0 z-20 -mx-4 mb-4 flex h-14 items-center justify-between border-b border-pit-border bg-pit-bg/95 px-4 backdrop-blur sm:static sm:mx-0 sm:mb-6 sm:rounded-xl sm:border sm:bg-pit-surface/85">
        {step === 0 ? (
          ...
          <button
            type="button"
            className="flex h-9 w-9 items-center justify-center rounded-lg text-pit-text transition hover:bg-white/5 hover:text-white"
            onClick={backStep}
            aria-label="Go back"
          >
            <ArrowLeft size={18} />
        ...
        <div className="text-center">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-pit-muted">
            Step {step + 1} of {steps.length}
          </p>
        </div>
        <button
          type="button"
          className="flex h-9 w-9 items-center justify-center rounded-lg text-pit-muted transition hover:bg-white/5 hover:text-white"
          onClick={onBack}
      ...
      <form id="create-tourney" onSubmit={submit} className="mt-6 space-y-5">
        {error && (
          <p className="rounded-xl border border-red-400/30 bg-red-400/10 px-4 py-3 text-sm font-semibold text-red-200">
            {error}
          </p>
        )}
        ...
        <section className="overflow-hidden rounded-2xl border border-pit-border bg-pit-surface/80 shadow-[0_24px_60px_rgba(0,0,0,0.28)]">
          <div className="border-b border-pit-border bg-gradient-to-r from-pit-teal/10 to-transparent px-4 py-4 sm:px-6">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-pit-teal">
              {steps[step]}
            </p>
            {(step !== 1 || isCashGame) && (
              <>
                ...
                <p className="mt-1 text-sm leading-5 text-pit-muted">
                  {step === 0 &&
                    "Start with the essentials. You can fine-tune everything later."}
                  {step === 1 && "Set the stakes, seats, and buy-in range."}
                  {step === 2 &&
                    (isCashGame
                      ? "Choose visibility and alerts."
                      : "Choose structure, tracking, and announcements.")}
                  {step === 3 &&
                    "Give it one last look before it goes on the calendar."}
                </p>
              </>
            )}
          </div>
          ...
          <div className="p-4 sm:p-6">
            {onboardingActive && !isCashGame && (
              <div className="mb-4 flex gap-3 rounded-xl border border-pit-teal/25 bg-pit-teal/[0.07] px-3 py-3 text-xs leading-5 text-pit-text">
                <CheckCircle2
                  className="mt-0.5 shrink-0 text-pit-teal"
                  size={16}
                />
                <p>
                  {step === 0 &&
                    "Pick the group that will host this game. Only groups you administer are shown."}
                  {step === 1 &&
                    "Start simple. Turn on rebuys or add-ons only when the game uses them."}
                  {step === 2 &&
                    "Saved blind structures appear here, or the calculator can build one after creation."}
                  {step === 3 &&
                    "After creation, use Players for check-in and Run Tournament when the room is ready."}
                </p>
              </div>
            )}
              ...
              <div className="space-y-5">
                ...
                >
                  ...
                  <GameTypeCard
                    ...
                    onClick={() =>
                      setForm((current) => ({ ...current, gametype: "cash" }))
                    }
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Field
                    label={isCashGame ? "Date (optional)" : "Date"}
                    error={
                      attemptedStep && !isCashGame && !form.tourneydate
                        ? "Required"
                        : undefined
                    }
                  >
                    <input
                      ...
                      type="date"
                      value={form.tourneydate}
                      onChange={set("tourneydate")}
                    />
                  </Field>
                  <Field
                    label={isCashGame ? "Time (optional)" : "Time"}
                    error={
                      attemptedStep && !isCashGame && !form.tourneytime
                        ? "Required"
                        : undefined
                    }
                  >
                    <QuarterHourTimeSelect
                      value={form.tourneytime}
                      onChange={(value) =>
                        setForm((current) => ({
                          ...current,
                          tourneytime: value,
                ...
                >
                  <SelectShell>
                    <select
                      className="input h-12 pr-10 font-semibold"
                      value={form.groupid}
                ...
                {groups.length === 0 && (
                  <p className="rounded-xl border border-pit-gold/30 bg-pit-gold/10 px-3 py-2.5 text-xs leading-5 text-pit-gold">
                    Create a group or ask a group admin to promote you before
                    hosting a game.
                  </p>
                )}
              </div>
              ...
              (isCashGame ? (
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <Field
                      label="Stakes"
                      error={
                        attemptedStep && !form.stakeslabel.trim()
                          ? "Required"
                          : undefined
                      }
                    >
                      <input
                        ...
                        placeholder="$1 / $2"
                        value={form.stakeslabel}
                        onChange={set("stakeslabel")}
                      />
                    </Field>
                    <Field label="Seats">
                      <input
                  ...
                  <WizardPanel
                    ...
                    title="Buy-in range"
                    description="Optional minimum and maximum amounts."
                  >
                    <div className="grid grid-cols-2 gap-3">
                      <Field label="Minimum">
                        <input
                          ...
                          placeholder="$0"
                          value={form.minbuyin}
                          onChange={set("minbuyin")}
                        />
                      </Field>
                      <Field
                        label="Maximum"
                        error={
                          attemptedStep && !cashRangeComplete
                            ? "Below minimum"
                            : undefined
                        }
                      >
                  ...
                  <Field label="Game notes">
                    <textarea
                      ...
                      placeholder="Optional location, house rules, or reminders"
                      value={form.cashnotes}
                      onChange={set("cashnotes")}
                    />
                  </Field>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  <WizardPanel
                    icon={<Users size={18} />}
                    title="Field size"
                    description="Leave it open or cap the number of seats."
                    error={
                      attemptedStep && !maxPlayersComplete
                        ? "Choose Unlimited or set a player cap."
                        : undefined
                    }
                  >
                    <SegmentedPicker
                      ...
                      onChange={(value) =>
                        setForm((current) => ({
                          ...current,
                          maxplayersmode: value,
                          maxplayers:
                            value === "unlimited" ? "" : current.maxplayers,
                        }))
                      }
                    />
                  ...
                  <ToggleRow
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        rebuysenabled: event.target.checked,
                      }))
                    }
                    title="Allow rebuys"
                  ...
                  {rebuysActive && (
                    <WizardPanel
                      inset
                      error={
                        attemptedStep && !rebuyDetailsComplete
                          ? "Add rebuy chips and the final eligible level."
                          : undefined
                      }
                    >
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <Field label="Chips">
                          <input
                            ...
                            onChange={set("rebuychips")}
                          />
                        </Field>
                      </div>
                      <Field label="Available through level" className="mt-3">
                        <input
                          className="input h-11"
                          type="number"
                          min="1"
                  ...
                  <ToggleRow
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        addonsenabled: event.target.checked,
                      }))
                    }
                  ...
                  {addonsActive && (
                    <WizardPanel
                      inset
                      error={
                        attemptedStep && !addonDetailsComplete
                          ? "Add the add-on chip amount."
                          : undefined
                      }
                    >
                      <div className="grid grid-cols-2 gap-3">
            ...
            {step === 2 &&
              (isCashGame ? (
                <div className="space-y-4">
                  <WizardPanel
                    icon={<Users size={18} />}
                    title="Who can see this game?"
                  >
                    <SegmentedPicker
                      value={form.visibility}
                      options={[
                        { value: "group_public", label: "Entire group" },
                        { value: "invite_only", label: "Invite only" },
                      ]}
                      onChange={(value) =>
                        setForm((current) => ({
                          ...current,
                          visibility: value as "group_public" | "invite_only",
                        }))
                      }
                    />
                  </WizardPanel>
                  {form.visibility === "invite_only" && (
                    <WizardPanel
                      ...
                      description={`${form.inviteUserIds.length} selected`}
                      error={
                        attemptedStep && !inviteComplete
                          ? "Pick at least one group member."
                          : undefined
                      }
                    >
                      <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
                        {inviteMembers.map((member) => {
                          const checked = form.inviteUserIds.includes(
                            member.userid,
                          );
                          return (
                            <label
                              key={member.userid}
                              className={`flex min-h-11 items-center gap-3 rounded-lg border px-3 py-2 text-sm font-semibold transition ${checked ? "border-pit-teal/50 bg-pit-teal/10 text-white" : "border-pit-border bg-pit-bg/45 text-pit-text"}`}
                            >
                              <input
                                type="checkbox"
                                ...
                                onChange={(event) =>
                                  setForm((current) => ({
                                    ...
                                    inviteUserIds: event.target.checked
                                      ? [
                                          ...current.inviteUserIds,
                                          member.userid,
                                        ]
                                      : current.inviteUserIds.filter(
                                          (id) => id !== member.userid,
                                        ),
                                  }))
                                }
                  ...
                  <ToggleRow
                    ...
                    title="Alert eligible players"
                    description="Send the game announcement by email and push."
                    icon={<Bell size={18} />}
                  />
                </div>
              ) : (
                <div className="space-y-4">
                  <WizardPanel
                    ...
                    description={
                      selectedStructure
                        ...
                        : "Use a saved structure or build one after creation."
                    }
                  >
                    <SelectShell>
                      <select
                        className="input h-12 pr-10 font-semibold"
                        value={form.savedstructureid}
                        onChange={set("savedstructureid")}
                  ...
                  <WizardPanel
                    ...
                    description={
                      canUseClubFeatures
                        ? "Choose who records rebuys, add-ons, and knockouts."
                        : "Standard host tracking is active."
                    }
                  >
                    <SegmentedPicker
                      value={form.playerselftracking ? "player" : "standard"}
                      options={[
                        { value: "standard", label: "Host tracks" },
                        { value: "player", label: "Players track" },
                      ]}
                      ...
                      onChange={(value) =>
                        setForm((current) => ({
                          ...current,
                          playerselftracking:
                            canUseClubFeatures && value === "player",
                        }))
                      }
                    />
                  </WizardPanel>
                  ...
                  <ToggleRow
                    ...
                    title="Register me"
                    description="Enable to register self to this game"
                  ...
                  <ToggleRow
                    ...
                    disabled={!form.groupid}
                    onChange={set("notifygroup")}
                    title="Announce to the group"
                    description="Email approved group members when the tournament is posted."
                    icon={<Bell size={18} />}
                  />
                </div>
              ))}
            ...
            {step === 3 && (
              <div className="space-y-4">
                <div className="flex items-start gap-3 rounded-xl border border-pit-teal/25 bg-gradient-to-br from-pit-teal/15 to-pit-bg/30 p-4">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-[0.18em] text-pit-teal">
                ...
                <SummarySection title="Basics" onEdit={() => goToStep(0)}>
                  <ReviewRow
                    ...
                    value={
                      form.tourneydate
                        ? formatHeroLongDate(form.tourneydate)
                        : "Date TBD"
                    }
                  />
                  <ReviewRow
                    ...
                    value={
                      form.tourneytime
                        ? formatTime12Hour(form.tourneytime)
                        : "Time TBD"
                    }
                  />
                </SummarySection>
                ...
                <SummarySection title="Game details" onEdit={() => goToStep(1)}>
                  {isCashGame ? (
                    <>
                      <ReviewRow
                        label="Stakes"
                        value={form.stakeslabel || "Not set"}
                      />
                      <ReviewRow
                        label="Seats"
                        value={form.seatsavailable || "Open"}
                      />
                      <ReviewRow
                        label="Buy-in range"
                        value={`${form.minbuyin ? `$${form.minbuyin}` : "No minimum"} - ${form.maxbuyin ? `$${form.maxbuyin}` : "No maximum"}`}
                      />
                    </>
                  ) : (
                    <>
                      <ReviewRow
                        label="Buy-in"
                        value={`$${Number(form.buyin || 0).toFixed(2)}`}
                      />
                      <ReviewRow
                        label="Rake"
                        value={`$${Number(form.rake || 0).toFixed(2)}`}
                      />
                      <ReviewRow
                        label="Max players"
                        value={maxPlayersReview || "Not set"}
                      />
                      <ReviewRow
                        label="Rebuys"
                        value={
                          rebuysActive
                            ? `$${Number(form.rebuyprice || 0).toFixed(2)} for ${Number(form.rebuychips || 0).toLocaleString()} through level ${form.rebuylastlevel}`
                            : "Off"
                        }
                      />
                      <ReviewRow
                        ...
                        value={
                          addonsActive
                            ? `$${Number(form.addonprice || 0).toFixed(2)} for ${Number(form.addonchips || 0).toLocaleString()}`
                            : "Off"
                        }
                      />
                    </>
                  ...
                  {isCashGame ? (
                    <>
                      <ReviewRow
                        ...
                        value={
                          form.visibility === "invite_only"
                            ? `${form.inviteUserIds.length} invited players`
                            : "Entire group"
                        }
                      />
                      <ReviewRow
                        label="Announcements"
                        value={form.notifygroup ? "On" : "Off"}
                      />
                    </>
                  ) : (
                    <>
                      <ReviewRow
                        ...
                        value={
                          selectedStructure?.name ||
                          "Use calculator after creation"
                        }
                      />
                      <ReviewRow
                        label="Stats tracking"
                        value={
                          form.playerselftracking
                            ? "Players track"
                            : "Host tracks"
                        }
                      />
                      <ReviewRow
                        label="Register me"
                        value={form.registerself ? "Yes" : "No"}
                      />
                      <ReviewRow
                        label="Group announcement"
                        value={form.groupid && form.notifygroup ? "On" : "Off"}
                      />
                    </>
                  )}
                </SummarySection>
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-pit-teal/25 bg-[#0d1115]/96 px-4 pb-[calc(12px+env(safe-area-inset-bottom))] pt-3 shadow-[0_-16px_36px_rgba(0,0,0,0.48)] backdrop-blur sm:static sm:rounded-xl sm:border sm:border-pit-border sm:bg-pit-surface/90 sm:p-3 sm:shadow-none">
          <div className="mx-auto flex w-full max-w-2xl items-center gap-3">
            <button
              type="button"
              className="btn-ghost h-12 min-w-[6.5rem] flex-1 justify-center gap-2"
              onClick={backStep}
              disabled={loading}
            >
              {step === 0 ? (
            ...
            {step < steps.length - 1 ? (
              <button
                type="button"
                className="btn-primary h-12 flex-[1.45] justify-center gap-2"
                onClick={advanceStep}
              >
              ...
              <button
                type="submit"
                className="btn-primary h-12 flex-[1.45] justify-center gap-2 bg-emerald-500 hover:bg-emerald-400"
                disabled={loading || !basicsComplete || !canAdvance}
              >
                {loading ? (
                  "Creating..."
                ) : (
                  <>
                    <CheckCircle2 size={17} /> Create Event
                  </>
                )}
              </button>
            )}
          ...
          {attemptedStep && !canAdvance && step < steps.length - 1 && (
            <p className="mt-2 text-center text-xs font-semibold text-pit-gold">
              Finish the highlighted settings to continue.
            </p>
          )}
        </div>
      </form>
...
function WizardStepper({
  ...
  return (
    <nav
      aria-label="Game setup progress"
      className="rounded-xl border border-pit-border bg-pit-surface/70 px-3 py-3 sm:px-5"
    >
        ...
        {steps.map((label, index) => {
          ...
          return (
            ...
            >
              {index > 0 && (
                <span
                  className={`absolute right-1/2 top-[13px] h-px w-full ${index <= activeStep ? "bg-pit-teal" : "bg-pit-border"}`}
                  aria-hidden="true"
                />
              )}
              <button
              ...
              >
                <span
                  className={`flex h-7 w-7 items-center justify-center rounded-full border text-[11px] font-black transition ${
                    complete || active
                ...
                <span
                  className={`max-w-full truncate text-[9px] font-black uppercase tracking-[0.08em] sm:text-[10px] ${active ? "text-white" : "text-pit-muted"}`}
                >
                  {label}
                </span>
              </button>
...
function GameTypeCard({
  ...
  const iconClasses =
    tone === "violet"
      ...
      : "border-pit-teal/30 bg-pit-teal/10 text-pit-teal";
  return (
    <button
      type="button"
      aria-pressed={active}
    ...
    >
      <span
        className={`flex h-12 w-12 items-center justify-center rounded-xl border ${iconClasses}`}
      >
        {icon}
      </span>
      <span className="mt-3 text-sm font-black text-white">{title}</span>
      <span className="mt-1 max-w-32 text-[11px] leading-4 text-pit-muted">
        {description}
      </span>
    </button>
  );
...
function SelectShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative">
      {children}
      <ChevronDown
        className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-pit-muted"
        size={17}
      />
    </div>
  );
...
function WizardPanel({
  ...
  return (
    <section
      className={`rounded-xl border p-3.5 ${error ? "border-red-400/40 bg-red-400/[0.05]" : inset ? "border-pit-border bg-pit-bg/30" : "border-pit-border bg-pit-bg/45"}`}
    >
      {(title || description || icon) && (
        <div className="mb-3 flex items-start gap-3">
          {icon && (
            <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-pit-teal/10 text-pit-teal">
              {icon}
            </span>
          )}
          <div className="min-w-0 flex-1">
            ...
            {description && (
              <p className="mt-0.5 text-xs leading-4 text-pit-muted">
                {description}
              </p>
            )}
          </div>
...
function ToggleRow({
  ...
  return (
    <label
      className={`flex min-h-[4.5rem] cursor-pointer items-center justify-between gap-4 rounded-xl border border-pit-border bg-pit-bg/45 px-3.5 py-3 transition hover:border-white/20 ${disabled ? "cursor-not-allowed opacity-55" : ""}`}
    >
      <div className="flex min-w-0 items-center gap-3">
        {icon && (
          <div
            className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white/[0.04] ${accent}`}
          >
            {icon}
          </div>
        )}
      ...
      <div
        className={`flex h-6 w-11 shrink-0 items-center rounded-full px-1 transition-colors duration-150 ${switchColor}`}
      >
        <div
          className={`h-4 w-4 rounded-full bg-white shadow transition-transform duration-150 ${checked ? "translate-x-4" : "translate-x-0"}`}
        />
      </div>
      <input
        type="checkbox"
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-pit-border bg-pit-bg/45">
      <div className="flex items-center justify-between border-b border-pit-border px-3.5 py-2.5">
        <h3 className="text-xs font-black uppercase tracking-[0.16em] text-pit-muted">
          {title}
        </h3>
        <button
          type="button"
          className="text-xs font-black text-pit-teal hover:text-white"
          onClick={onEdit}
        >
          Edit
        </button>
...
function ReviewRow({
  ...
  return (
    <div className="flex min-h-11 items-center justify-between gap-4 py-2.5 text-sm">
      <span className="flex min-w-0 items-center gap-2 text-pit-muted">
        {icon && <span className="text-pit-teal">{icon}</span>}
        {label}
      </span>
      <span className="max-w-[62%] text-right font-bold text-white">
        {value}
      </span>
    </div>
  );
...
function gameToScheduleItem(
  game: GameListItem,
): Extract<ScheduleItem, { kind: "cash" }> {
...
function buildLeagueScheduleItems(
...
function leagueEventToScheduleItem(event: LeagueScheduleEvent): ScheduleItem {
  return {
    ...
    running: isEnabledFlag(event.running),
    hasStarted: isEnabledFlag(event.hasstarted),
...
function getScheduleCommunityKey(item: ScheduleItem): string | null {
  ...
  if (item.kind === "cash")
    return item.game.groupid ? `group:${item.game.groupid}` : null;
...
function compareScheduleItems(a: ScheduleItem, b: ScheduleItem) {
  return getAppScheduleSortKey(a.date, a.time).localeCompare(
...
function isScheduleItemFinalized(item: ScheduleItem) {
  ...
  if (item.kind === "cash")
    return item.game.status === "completed" || item.game.status === "cancelled";
  return item.completed;
}

function isLiveScheduleItem(item: ScheduleItem) {
  if (isScheduleItemFinalized(item)) return false;
  if (item.kind === "cash") return item.game.status === "active";
  if (item.kind === "league") return isEnabledFlag(item.running);
  return isEnabledFlag(item.tournament.running);
}
...
function isUpcomingScheduleItem(item: ScheduleItem) {
  ...
  if (item.kind === "tournament" && isEnabledFlag(item.tournament.hasstarted))
    return true;
  if (item.kind === "league" && isEnabledFlag(item.hasStarted)) return true;

  return getAppScheduleSortKey(item.date, item.time) >= dashboardWallTime();
}
...
function nowInAppTimezone() {
  return dashboardWallTime();
}

function hasScheduleStarted(
  date: string | null | undefined,
  time: string | null | undefined,
) {
  if (!date) return false;
  const effectiveTime = (time?.slice(0, 8) ?? "00:00:00")
    .padEnd(8, ":00")
    .slice(0, 8);
